...
Sistemin dışarıya açılan kapısıdır. Tüm HTTP/HTTPS isteklerini ve WebSocket sinyalleşme trafiğini alır, uygun servislere (bbb-web, bbb-html5, FreeSWITCH vb.) yönlendirir (reverse proxy). Astatik dosyaları (HTML, CSS, JS) istemcilere sunar.
...
- `bbb-web`, `bbb-html5` ve `bbb-apps-akka` arasındaki iletişimi sağlar (Pub/Sub mimarisi uzerinden).
